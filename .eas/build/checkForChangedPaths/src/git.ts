...
import { core, getExecOutput } from './paths-filter-deps';
...
export async function getChangesSinceMergeBase(
  ...
  async function hasMergeBase(): Promise<boolean> {
    ...
    return (
      (
        await getExecOutput('git', ['merge-base', baseRef, headRef], {
    ...
    if (!(await hasMergeBase())) {
      ...
      if (baseRef === undefined || headRef === undefined) {
        ...
        if (baseRef === undefined || headRef === undefined) {
          await getExecOutput(
            'git',
            ['fetch', '--tags', '--depth=1', 'origin', base, head],
            {
...
export async function getCurrentRef(): Promise<string> {
  ...
  try {
    const branch = (
      await getExecOutput('git', ['branch', '--show-current'])
    ).stdout.trim();
    if (branch) {
      return branch;
    }

    const describe = await getExecOutput(
      'git',
...
async function hasCommit(ref: string): Promise<boolean> {
  return (
    (
      await getExecOutput('git', ['cat-file', '-e', `${ref}^{commit}`], {
        ignoreReturnCode: true,
      })
    ).exitCode === 0
  );
...
async function getCommitCount(): Promise<number> {
  const output = (await getExecOutput('git', ['rev-list', '--count', '--all']))
    .stdout;
...
async function getLocalRef(shortName: string): Promise<string | undefined> {
  ...
  const output = (
    await getExecOutput('git', ['show-ref', shortName], {
      ignoreReturnCode: true,
    })
  ).stdout;
  const refs = output
    .split(/\r?\n/g)
    .map((l) =>
      l.match(/refs\/(?:(?:heads)|(?:tags)|(?:remotes\/origin))\/(.*)$/),
    )
